...
LFM_hist <- lfm_raw %>%
  ...
  separate(collection_date, sep = "/", into = c("month", "day", "year")) %>%
  mutate(month1 = month(mdy(collection_date2), label = TRUE, abbr = FALSE)) %>%
...
LFM_2022 <- lfm_raw %>%
  filter(concern != "Yes",
         ...
         site == "Painted Cave") %>%
  mutate(collection_date2 = collection_date) %>%
  separate(collection_date2, sep = "/", into = c("month", "day", "year")) %>%
  filter(year == "2022" | year == "2023") %>%
...
min_2022_date <- as_date("2022-05-30")
max_2022_date <- as_date("2023-05-30")
...
ggsave(plot = LFM2022_big, filename = "Figures/LFM_2021_04_15_Big.png", height = 8.5, width = 11, units = "in", dpi = 300)
ggsave(plot = LFM2022_small, filename = "Figures/LFM_2021_04_15_Small.png", height = 4, width = 6, units = "in", dpi = 300)
